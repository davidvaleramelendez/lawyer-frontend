...
const InvoiceAdd = () => {
  ...
  const handleReset = () => {
    ...
    if (store.invoiceItem.case && store.invoiceItem.case.CaseID) {
      ...
      if (store.invoiceItem.case.CaseTypeID) {
        if (store.caseTypeItems && store.caseTypeItems.length) {
          const index = store.caseTypeItems.findIndex(x => x.CaseTypeID === store.invoiceItem.case.CaseTypeID)
          if (index !== -1) {
            invoiceItem.CaseTypeName = {
              value: store.caseTypeItems[index].CaseTypeID,
              label: store.caseTypeItems[index].CaseTypeName,
              id: store.caseTypeItems[index].CaseTypeID
            }
          }
        }
      }
    ...
    if (store.invoiceItem.method) {
      ...
      if (index !== -1) {
        ...
        invoiceItem.method = paymentMethod[0]
      }
    }
  ...
  useEffect(() => {
    ...
    /* For reset form data */
    if (store && store.actionFlag && store.actionFlag === "INVOICE_INFO") {
      handleReset()
    }

    /* Redirect to listing */
    if (store && store.actionFlag && store.actionFlag === "CREATED") {
      if (store.invoiceItem && store.invoiceItem.id) {
        navigate(`${adminRoot}/invoice/view/${store.invoiceItem.id}`)
      } else {
  ...
  const onSubmit = async (values) => {
    if (values) {
      ...
      dispatch(saveInvoiceItem(invoiceData))
    }
  }
